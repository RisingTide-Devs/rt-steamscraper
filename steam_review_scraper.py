...
Reads app IDs from app_ids.txt
"""
...
def scrape_app(app_id, db):
    """Scrape reviews for a single app ID"""
...
try:
    ...
    print("Create a file called 'app_ids.txt' with one app ID per line")
    sys.exit(1)
...
for i, app_id in enumerate(app_ids, 1):
    print(f"\n{'='*80}")
    print(f"Processing app {i}/{len(app_ids)}: {app_id}")
    print(f"{'='*80}")
    
    try:
        count = scrape_app(int(app_id), db)
        total_reviews += count
    except Exception as e:
...
print(f"\n{'='*80}")
print(f"All done! Total reviews saved: {total_reviews}")
print(f"{'='*80}")